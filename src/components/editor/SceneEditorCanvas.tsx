...
const SceneEditorCanvas = forwardRef<SceneEditorHandle, SceneEditorCanvasProps>(
  function SceneEditorCanvas(
    ...
    const [skyboxUrl, setSkyboxUrl] = useState<string | null>(
      sceneGraph?.skybox?.textureUrl ?? null
    );
    const [skyboxHeight, setSkyboxHeight] = useState<number>(
      sceneGraph?.skybox?.height ?? 15
    );
    const [skyboxRadius, setSkyboxRadius] = useState<number>(
      sceneGraph?.skybox?.radius ?? 100
    );
    const [skyboxResolution, setSkyboxResolution] = useState<number>(
      sceneGraph?.skybox?.resolution ?? 32
    );
    const [skyboxLoading, setSkyboxLoading] = useState(false);
    const [localSkyboxes, setLocalSkyboxes] = useState<
      Array<{ label: string; textureUrl: string }>
    >([]);
    ...
    const availableSkyboxes = useMemo(() => {
      ...
      localSkyboxes.forEach((option) => {
        if (
          !merged.some((existing) => existing.textureUrl === option.textureUrl)
        ) {
          merged.push(option);
        }
      });
      ...
      const loadLocalSkyboxes = async () => {
        try {
          ...
          const data = (await response.json()) as {
            skyboxes?: Array<{ label: string; textureUrl: string }>;
          };
    ...
    const disposeGroundedSkybox = useCallback(() => {
      ...
      if (grounded) {
        const material = (grounded as unknown as {
        ...
        (
          grounded as unknown as { geometry?: THREE.BufferGeometry }
        ).geometry?.dispose?.();
      }
    ...
    useEffect(() => {
      const sceneNode = sceneGraph?.scene;
      if (sceneNode && sceneNode.background instanceof THREE.Color) {
        sceneNode.background.set(backgroundColor);
      }
    ...
    useEffect(() => {
      ...
      characters.forEach((character) => {
        const helper = new THREE.Box3Helper(
          new THREE.Box3(),
          new THREE.Color(character.bounds?.color ?? 0x22c55e)
        );
    ...
    const captureFrame = useCallback((): SceneEditorCapture | null => {
      ...
      if (transformHelperRef.current) {
        hiddenNodes.push({
          object: transformHelperRef.current,
          visible: transformHelperRef.current.visible,
        });
        transformHelperRef.current.visible = false;
      }

      boundingHelpersRef.current.forEach((helper) => {
        hiddenNodes.push({ object: helper, visible: helper.visible });
      ...
      hiddenNodes.forEach(({ object, visible }) => {
          ...
          <Card className="pointer-events-auto border border-zinc-200/80 bg-white/90 shadow-sm backdrop-blur">
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs leading-relaxed">
                ...
                transforms, and selection programmatically. The viewport supports orbit (left drag), pan (right drag),
                and zoom (wheel or pinch).
              </CardDescription>
            </CardHeader>
              ...
              <div className="rounded-lg border border-zinc-200/80 bg-white/70 p-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-2 rounded-md border border-zinc-200/70 px-3 py-2">
                    ...
                    <Checkbox
                      checked={showBounds}
                      onCheckedChange={(checked) => setShowBounds(checked === true)}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="w-12 text-right font-mono text-[12px] text-zinc-700">
                        {exposure.toFixed(2)}
                      </span>
                    </div>
                  </div>
          ...
          <Card className="pointer-events-auto border border-zinc-200/80 bg-white/90 shadow-sm backdrop-blur">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                {skyboxLoading && (
                  <span className="text-[11px] font-semibold text-amber-600">Loading…</span>
                )}
              </div>
              <CardDescription className="text-xs text-zinc-600">
                Swap PNGs from <code className="rounded bg-zinc-100 px-1">/public/assets/skyboxes</code> or remote URLs,
                and tweak grounded parameters.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-zinc-800">
              ...
              <div className="space-y-2 text-xs text-zinc-600">
                <div className="flex items-center gap-2">
                  <span className="w-16 text-[11px] font-semibold uppercase tracking-wide text-zinc-500">
                    Height
                  </span>
                  <input
                    type="range"
                    ...
                    className="h-2 w-full cursor-pointer accent-sky-500"
                  />
                  <span className="w-10 text-right font-mono text-[12px] text-zinc-700">
                    {skyboxHeight.toFixed(0)}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-16 text-[11px] font-semibold uppercase tracking-wide text-zinc-500">
              ...
              <div className="rounded-md bg-emerald-50 p-2 text-[11px] text-emerald-800">
                Character objects tagged with{" "}
                <code className="rounded bg-emerald-100 px-1">type:character</code>{" "}
                render live bounding boxes for collision and layout checks.
              </div>
